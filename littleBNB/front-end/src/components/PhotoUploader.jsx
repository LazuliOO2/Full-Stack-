...
const PhotoUploader = ({ photolink, setPhotoLink, setPhotos, photos }) => {
  ...
  const uploadPhoto = (ev) => {
    ...
    for (let i = 0; i < files.length; i++) {
      data.append('photos', files[i]); // 'photos' deve bater com o nome no multer do back-end
    }

    // Envia para o back-end com o header correto
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="mt-2 grid grid-cols-5 gap-4">
        ...
        >
          <input type="file" multiple id="file" className="hidden" onChange={uploadPhoto} />
          <svg
